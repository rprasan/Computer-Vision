...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #Flatten the training and the testing images
    xTr=torch.flatten(x_train,start_dim=1)
    xTs=torch.flatten(x_test,start_dim=1)
    #Loop over the train and the test sets to compute the Euclidean distances
    for i,tr in enumerate(xTr):
        for j,ts in enumerate(xTs):dists[i,j]+=torch.sum((tr-ts)**2)**0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xTr=torch.flatten(x_train,start_dim=1)
    xTs=torch.flatten(x_test,start_dim=1)
    #Loop over the train set to compute the Euclidean distances
    for i,tr in enumerate(xTr):dists[i,:]=torch.sum((tr-xTs)**2,dim=1)**0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    xTr=torch.flatten(x_train,start_dim=1)
    xTs=torch.flatten(x_test,start_dim=1)
    #Compute the Euclidean distances - (x2-x1)^2+(y2-y1)^2=x1^2+x2^2-2x1x2+y1^2+y2^2-2y1y2
    xTr2=torch.sum(torch.square(xTr),1,keepdim=True)       #x1^2+y1^2
    xTs2=torch.sum(torch.square(xTs),1,keepdim=True)       #x2^2+y2^2
    dists=torch.sqrt(xTr2+xTs2.t()-2*torch.mm(xTr,xTs.t()))#3rd term is -2(x1x2+y1y2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    #Get the row numbers of the 'k' largest distances along each column
    _,idx=torch.topk(dists,k=k,dim=0,largest=False)
    idx=idx.view(k,num_test)
    #Loop over each column of 'idx' to perform voting
    for i in range(num_test):y_pred[i],_=torch.mode(y_train[idx[:,i]])
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
        acc=[]                          #fold accuracies for current 'k'
        #Loop over all 'K' folds
        for fold in range(num_folds):
            #Define the validation set by choosing one fold
            xVal=x_train_folds[fold]
            yVal=y_train_folds[fold]
            #Define the training set by concatenating different folds
            if fold==0:                 #the first fold is the validation set
                xTr=torch.cat(x_train_folds[1:])
                yTr=torch.cat(y_train_folds[1:])
            elif fold==num_folds-1:     #the last fold is the validation set
                xTr=torch.cat(x_train_folds[:fold])
                yTr=torch.cat(y_train_folds[:fold])
            else:                       #the validation set is neither the first nor the last fold
                xTr=torch.cat((torch.cat(x_train_folds[:fold]),
                               torch.cat(x_train_folds[fold+1:])))
                yTr=torch.cat((torch.cat(y_train_folds[:fold]),
                               torch.cat(y_train_folds[fold+1:])))
            #Classificaiton accuracy for the current fold
            model=KnnClassifier(xTr,yTr)#the classification model
            acc.append(model.check_accuracy(xVal,yVal,k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    accAvg=[sum(item[1])/len(item[1]) for item in k_to_accuracies.items()]
    indices=[i for i,item in enumerate(accAvg) if item==max(accAvg)]
    best_k=torch.min(torch.tensor(list(k_to_accuracies.keys()))[indices]).item()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
